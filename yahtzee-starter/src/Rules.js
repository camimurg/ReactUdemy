...
class SmallStraight extends Rule{
  evalRoll = dice => {
    ...
    const hasThreeFour = d.has(3) && d.has(4)
    if( d.has(2) && hasThreeFour && (d.has(1) || d.has(5)) )
    return this.score;

    if( d.has(5) && hasThreeFour && (d.has(2) || d.has(6)) )
    return this.score;
    return 0
  };
}
...
const sixes = new TotalOneNumber({ val: 6, description: "6 points per 16" });
...
const yahtzee = new Yahtzee({ score: 50, description: "50 points for KNIFFEL" });
